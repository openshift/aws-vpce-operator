...
CI/CD validation and workflow integrity for AWS VPCE Operator.
...
**Parity validation:**
```bash
# Show golangci-lint version pinned in prek config
grep "rev:" .pre-commit-config.yaml | grep golangci-lint

# Show gitleaks version pinned in prek config
...
```bash
# Verify CI-critical checks exist in Tekton pipelines
TEKTON_CHECKS=(
  "golangci-lint"
  "gitleaks"
  "go test"
...
for check in "${TEKTON_CHECKS[@]}"; do
  if ! grep -q "$check" .tekton/*.yaml; then
    echo "WARNING: $check not found in Tekton pipelines"
  fi
done

# Verify prek-only hooks exist in prek config
for check in "rbac-wildcard-check"; do
  if ! grep -q "$check" .pre-commit-config.yaml; then
    echo "WARNING: $check not found in prek config"
  fi
done
```
...
# List all pinned tool versions in prek config
grep -E "rev:|repo:" .pre-commit-config.yaml

# Test container build (same as CI)
make docker-build

# Run in CI-equivalent environment (specify target, e.g. go-test, go-check)
...
Local Reproduction:
  make go-check
  # Output shows 3 linter errors in controllers/vpcendpoint/reconciler.go
