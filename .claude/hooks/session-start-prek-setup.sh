...
if ! command -v prek &> /dev/null; then
  cat >&2 <<'EOF'
+----------------------------------------------------------------+
|  prek (pre-commit hooks) is not installed                      |
|                                                                |
...
# Use git-common-dir for worktree compatibility
GIT_DIR=$(git rev-parse --git-common-dir 2>/dev/null || git rev-parse --git-dir 2>/dev/null)
if [[ -z "$GIT_DIR" ]]; then
  exit 0
fi

if [[ ! -f "$GIT_DIR/hooks/pre-commit" ]] || ! grep -q "prek" "$GIT_DIR/hooks/pre-commit" 2>/dev/null; then
  echo "Setting up prek pre-commit hooks..." >&2

  if prek install >/dev/null 2>&1; then
    echo "Pre-commit enabled" >&2
  else
    echo "Failed to install prek hooks - you may need to run 'prek install' manually" >&2
  fi
else
  echo "Pre-commit enabled" >&2
fi
