...
```
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_aws_vpce_operator/<PR_NUMBER>/pull-ci-openshift-aws-vpce-operator-master-lint/<BUILD_ID>
```
...
```bash
# View recent PR jobs
curl -s "https://prow.ci.openshift.org/?repo=openshift%2Faws-vpce-operator&type=presubmit" | grep -E "pull-ci-openshift-aws-vpce-operator"
...
- **Pull request jobs**: `gs://test-platform-results/pr-logs/pull/openshift_aws_vpce_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]`
- **Periodic jobs**: `gs://test-platform-results/logs/[JOB_NAME]/[JOB_ID]`

**Viewing logs via web**:
```text
https://prow.ci.openshift.org/view/gs/test-platform-results/pr-logs/pull/openshift_aws_vpce_operator/[PR_NUMBER]/[JOB_NAME]/[JOB_ID]
```
...
- `pull-ci-openshift-aws-vpce-operator-master-e2e-binary-build-success` - E2E binary build verification
- `pull-ci-openshift-aws-vpce-operator-master-coverage` - Code coverage analysis (with Codecov)
- `pull-ci-openshift-aws-vpce-operator-master-lint` - Linting checks
- `pull-ci-openshift-aws-vpce-operator-master-test` - Unit tests
- `pull-ci-openshift-aws-vpce-operator-master-validate` - Validation checks

**Tekton Pipelines** (configured in `.tekton/`):
- `aws-vpce-operator-pull-request` - Main PR pipeline (docker build with OCI-TA)
- `aws-vpce-operator-e2e-pull-request` - E2E testing pipeline
- `aws-vpce-operator-pko-pull-request` - PKO (Package Operator) pipeline
...
```text
https://prow.ci.openshift.org/?repo=openshift%2Faws-vpce-operator&type=presubmit
```
...
- Configuration: `ci-operator/config/openshift/aws-vpce-operator/openshift-aws-vpce-operator-master.yaml`
- Runs: lint, test, validate, coverage, e2e-binary-build
- Uses Codecov for coverage reporting (secret: `aws-vpce-operator-codecov-token`)
...
- Builds container images to Quay (aws-vpce-operator-tenant)
...
# Option 2: Use gh CLI to find URLs
gh pr checks <PR_NUMBER> | grep "pull-ci-openshift-aws-vpce-operator"
...
# https://prow.ci.openshift.org/?repo=openshift%2Faws-vpce-operator

# Or use platform-specific command:
# macOS: open "https://prow.ci.openshift.org/?repo=openshift%2Faws-vpce-operator"
# Linux: xdg-open "https://prow.ci.openshift.org/?repo=openshift%2Faws-vpce-operator"
# Windows: start "https://prow.ci.openshift.org/?repo=openshift%2Faws-vpce-operator"

# View specific PR on Prow (replace <PR_NUMBER>)
# https://prow.ci.openshift.org/?repo=openshift%2Faws-vpce-operator&type=presubmit&pull=<PR_NUMBER>
```
...
- Prow jobs: `pull-ci-openshift-aws-vpce-operator-master-*`
- Tekton jobs: Usually show as "Tekton" or pipeline names in PR checks
- Verify repo name format in Prow: `openshift_aws_vpce_operator` (underscore, not dash)
...
- Common issues:
  ...
  - Auth failures → Secret configuration in `aws-vpce-operator-tenant` namespace
- Local validation:
  ```bash
...
- Location: `ci-operator/config/openshift/aws-vpce-operator/openshift-aws-vpce-operator-master.yaml`
- Update process: Submit PR to openshift/release repository
- Auto-generated jobs in: `ci-operator/jobs/openshift/aws-vpce-operator/`

**Tekton Pipelines** (in this repo):
- Location: `.tekton/` directory
- Files:
  - `aws-vpce-operator-pull-request.yaml` - Main PR pipeline
  - `aws-vpce-operator-push.yaml` - Post-merge pipeline
  - `aws-vpce-operator-e2e-pull-request.yaml` - E2E testing
...
- Secret: `aws-vpce-operator-codecov-token` (stored in Prow)
- Generate coverage locally: `make coverage`
- Coverage runs on PRs and post-merge (`publish-coverage`)
- Dashboard: Check Codecov for aws-vpce-operator
