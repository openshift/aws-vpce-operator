...
Testing guidelines for the AWS VPCE Operator.
...
go test ./controllers/vpcendpoint/
...
**Minimal validation:**
```bash
# After changing controllers/vpcendpoint/
go test ./controllers/vpcendpoint/
```
...
```go
It("should create VPC endpoint", func() {
    vpcEndpoint := &v1alpha2.VpcEndpoint{...}
    Expect(k8sClient.Create(ctx, vpcEndpoint)).To(Succeed())

    _, err := reconciler.Reconcile(ctx, req)
    Expect(err).NotTo(HaveOccurred())

    endpoint := &v1alpha2.VpcEndpoint{}
    Expect(k8sClient.Get(ctx, endpointKey, endpoint)).To(Succeed())
    Expect(endpoint.Status.Status).To(Equal("Available"))
})
```
...
Prek hooks run gitleaks and lint checks automatically. Tests are NOT in prek config
(too slow for pre-commit). Run manually before pushing: `make go-test`
